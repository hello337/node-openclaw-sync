...
2. **Phase 0** (every 60s): Polls the provider's token endpoint using saved `device_code`. When user approves → saves token to `~/.openclaw/agents/main/agent/auth-profiles.json` → applies `config.patch` (provider config + `agents.defaults`) → notifies xNode.
